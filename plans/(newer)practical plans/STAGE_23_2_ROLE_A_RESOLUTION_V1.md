...
```text
1. commit STAGE_23_2_ROLE_A_RESOLUTION_V1.md alone
...
3. if the audit finds an executable contradiction
   - preserve committed V1 as historical
   - create V2
   - independently review and commit V2
   - do not execute until corrected plan is frozen
...
This does **not** establish whether they are biological replicates or technical lanes.

That question is explicitly reopened as a source-design question in 23.2A.
...
Rewind strata:
  ...
  crossed with n_lanes where possible
```
...
historical bootstrap artifact hash correctly
historical permutation artifact hash correctly
...
Resolve what the two control GSMs represent using this evidence hierarchy:
...
Clone overlap, barcode reuse, or count patterns may support interpretation but may **not** alone prove biological replication.

Output exactly one:

```text
REPLICATE_STRUCTURE_BIOLOGICAL
REPLICATE_STRUCTURE_TECHNICAL_LANES
...
Do not infer that two GSM accessions equal two biological replicates.
...
Historical anchor expected from the source reconstruction:

```text
top-N = 100
source tie behavior may yield >100 selected barcodes
historical cutoff anchor includes the previously observed rank-100 tie
```
...
The mapping table itself may be cached.
...
The core Stage-23.2B/C experiment is a paired 2×2 diagnostic:
...
MATCHED SEARCH    cell 10                cell 11
K fixed at 20
4 C candidates
```
...
cell 11:
  R1depth(Bdepth)
  R3depth matched search:
    K fixed at 20
    C ∈ {0.01,0.1,1,10}
```

Why `K=20` for the matched search:

```text
it is the middle value of the frozen K grid
and is chosen by protocol position, not because it performed best in Stage 23
```
...
source-design audit recorded
gDNA reconstruction inputs available
Bdepth exactly computable for all 3,147 clones
historical permutation mappings exactly recoverable
2×2 design executable in every fold
...
D10_j =
  ΔAP under cell 10
  = matched-search R3(K=20,B0) - R1(B0)

S_j =
  D00_j - D10_j
```

The historical committed `D00` values are authoritative.

Do not recompute them merely for convenience if the committed null array exists.

If only summary statistics were committed, the recovered mapping + historical pipeline must reproduce them before proceeding.
...
## 6.4 Conditional width-ladder diagnostic

Run this only if the primary status is `UNRESOLVED`.

This condition is pre-registered now.
...
for an 8-candidate R3 search.

Compare paired null means:

```text
4-candidate K=20 search
8-candidate K={10,20} search
12-candidate K={10,20,50} search
```

A monotonic increase is descriptive support.

It does not replace the primary paired CI rule.

## 6.5 Observed-data sensitivity

For context only, refit the **observed** Role-A comparison under the matched K=20 search using the frozen outer folds.

Report:

```text
matched-search observed ΔAP
historical full-search observed ΔAP
difference
```
...
CI
status
conditional-ladder status
observed matched-search sensitivity
```
...
```text
D11_j =
  matched-search R3depth(K=20,Bdepth) - R1depth(Bdepth)
```

This is the joint "selection restricted + depth expanded" null.

The full 2×2 null summary is:

```text
μ00 = mean(D00)  historical full search + B0
μ10 = mean(D10)  matched search          + B0
μ01 = mean(D01)  full search             + Bdepth
μ11 = mean(D11)  matched search          + Bdepth
```
...
O10 = observed matched-search         + B0 ΔAP
O01 = observed full-search            + Bdepth ΔAP
O11 = observed matched-search         + Bdepth ΔAP
```
...
It is forbidden if the GSMs are biological replicates or unresolved.
...
## 8.5 Cross-GSM measurement concordance

For clones with usable gDNA support in both control GSMs, report:

```text
Spearman(log1p gDNA support in GSM7092515,
         log1p gDNA support in GSM7092516)

top-100 selection agreement
positive / negative discordance counts
```

Interpretation depends on 23.2A:

```text
technical lanes:
  concordance is technical-reproducibility evidence

biological replicates:
  concordance mixes technical + biological reproducibility

unresolved:
  call it cross-GSM concordance only
```

Do not call it biological replication unless 23.2A established that.
...
Power analysis must use the 23.2A design status.
...
then the current benchmark is one source biological context as far as the audited metadata supports.
...
record the number and independence structure exactly, but remember both GSMs were already used in Stage 23 and are not untouched confirmation.
...
power conclusions about biological replication must also be marked unresolved.
...
## 9.5 Power-limitation status

Use the primary AUC=0.66 curve.

```text
POWER_LIMITATION = SUPPORTED
```
...
```text
POWER_LIMITATION = NOT_SUPPORTED
```
...
```text
POWER_LIMITATION = UNRESOLVED
```

If power remains <0.80 even at scale 4, report that the tested signal scale would require more information than the tested cohort-size range provides; do not extrapolate a precise required N from a failed range.

Any projected clone-count requirement is **not** a substitute for independent biological replication. 23.2G still requires untouched confirmation evidence.
...
That is sufficient for planning new confirmation evidence.
...
The first four use their frozen substage rules.
...
label NOT_SUPPORTED
power SUPPORTED
    -> plausible model-selection + power explanation
...
label NOT_SUPPORTED
power SUPPORTED
    -> multiple methodological/power mechanisms
...
whether the rare-positive design has low detection power for a controlled signal scale
what the two Rewind control GSMs represent, if source evidence resolves it
...
That stronger statement requires the failure decomposition to exclude plausible methodological, measurement, and power explanations.
...
```text
_cc_cache/stage23_2/
  recovered permutation mappings
  2×2 per-permutation diagnostic arrays
...
```text
23.2B matched-search null
23.2C full + matched Bdepth nulls
