...
Do **not** use bare `cellID` as a globally unique identifier across lanes.
...
Do not hard-code the current Windows paths into reusable loaders, tests, or benchmark tables. Manifests may record the path used for a run as non-portable provenance, but file identity is established by accession/basename/size/hash.
...
Construct one row per author-QC pretreatment cell/sample record.
...
The primary prospective benchmark population is:

```text
all author-QC clone-assigned pretreatment cells
```
...
If such exclusions are not required for the label definition, retain the cells and record the distinction.
...
Do not assume a 10x barcode string is globally unique across samples.
...
`post_fraction` is compositional within each treatment sample: clone fractions sum to one, so clone outcomes are not statistically independent within a treatment. Preserve this limitation explicitly; Stage 23 must not count the 8,406 clone×treatment rows as 8,406 independent biological units.
...
Create the split once on the **clone table**, using:
...
unique bare cellID
unique clones
cells per clone distribution
clones spanning both lanes
positive cells
negative cells
positive clones
negative clones
...
source git HEAD before benchmark generation
...
Keep committed result/manifests deterministic: do not inject wall-clock timestamps into content that is expected to reproduce byte-for-byte. If a run timestamp is useful, put it in the Stage-22 record/log rather than making it part of the benchmark identity.
...
no clone has contradictory outcome
42 primed records
35 primed clones
author top-100-with-ties rule is used
no bare-cellID global uniqueness assumption
all cells from a clone map to one outer_group and one outer_fold
...
Every eligible pretreatment record used by the benchmark maps to an expression column, or exclusions are explicitly enumerated and justified.
...
WM989:
  verdict
  naive cells/clones
  unique cell_uid / duplicate bare barcodes across samples
