...
**Status:** DRAFT, REPOSITORY-REVIEWED — commit this plan alone, then complete the independent data-machine pre-execution audit before fitting any model. No Stage-24 model architecture work may begin until Stage 23 is executed, frozen, and reviewed.  
**Plan version:** V1  
...
The plan is committed before the independent coding-agent/data-machine audit so the reviewed starting specification remains historical. That audit is **not** permission to fit models. If it finds an executable contradiction, archive committed V1, create V2, review/commit V2, and only then execute 23A–23F.
...
For C1, all 1,401 outer-training/test-eligible clones remain represented by six treatment rows.
...
treatment dummy coding exactly frozen
...
pooled ΔAP > 0
95% CI lower bound > 0
R3 beats R1 in >=3/5 outer folds
```
...
```text
W5 = X+B+U+X×U
acid is frozen reference treatment with five non-reference dummies
...
```text
1. commit Stage-23 V1 plan alone

2. independent pre-execution audit only
   read V1 end to end
   check it against committed Stage-22 artifacts and external raw matrices
   do not fit models
   do not edit committed V1 in place
   STOP and report whether V1 is executable exactly as written

3. if the audit finds a contradiction
   archive V1
   create/review/commit V2
   do not execute until the corrected version is frozen

4. 23A
   run the executable §3.0 input/hash audit
